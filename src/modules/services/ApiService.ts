...
import { delay, map, retryWhen, switchMap, take, concat } from 'rxjs/operators';
...
export class ApiService {
  ...
  public createTodo(todo: TodoModel.ITodo): Observable<void> {
    return from(
      this.fbApp
        .database()
        .ref('/todos')
  ...
  public getTodoList(query: GeneralModel.IApiQuery): Observable<GeneralModel.IPagination<TodoModel.ITodo>> {
    ...
    const subject = new Subject<GeneralModel.IPagination<TodoModel.ITodo>>();
    this.fbApp
      .database()
      .ref('/todos')
      .once('value', snapshot => {
